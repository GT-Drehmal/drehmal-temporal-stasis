...
class Chunk(Chunk):  # type: ignore
    ...
    def __init__(self, nbt_data: nbt.NBTFile):
        ...
        try:
            ...
            Chunk._logger.debug("Chunk data version is pre-15w32a")
...
def restore_world() -> int:
    ...
    try:
        ...
        if (
            ...
            if os.path.exists(claims_dir):
                ...
                claims_dir = os.path.join(
                    os.path.dirname(parsed_args.active), "data", "openpartiesandclaims", "player-claims"
                )
                if os.path.exists(claims_dir):
                    ...
                    if os.path.exists(claims_dir):
                        ...
                        logger.error(
                            "Argument '-claims' was indicated, but the player-claims folder was not found! Is your directory structure strange? Aborting..."
                        )
                        return -1
        ...
        if parsed_args.preview:
            logger.warning("Running under PREVIEW mode. No changes will be committed to the files.")
    ...
    logger.info(
        f"Restored the {dimension} at {parsed_args.active} to its original state at {parsed_args.original} in {time.perf_counter() - start_time:.3f} seconds"
    )
    return 0


def process_region(
    original_region_dir, active_region_dir, original_entities_dir, active_entities_dir, claimed_chunks, idx, region_file
):
    ...
    if not os.path.exists(original_region_path):
        logger.warning(
            f"({idx}/{len(os.listdir(active_region_dir))}) Region {region_file} does not exist in original world, skipping."
        )
        return
    ...
    if parsed_args.boundary:
        ...
        if not (min_x <= regional_x <= max_x and min_z <= regional_z <= max_z):
            logger.info(
                f"({idx}/{len(os.listdir(active_region_dir))}) Region ({regional_x}, {regional_z}) not within allowed boundary ({min_x}, {min_z}) - ({max_x}, {max_z}), skipping."
            )
            return
        ...
        for chunk_z in range(32):
            ...
            try:
                ...
                if active_region_chunk:
                    ...
                    logger.debug("Keeping active chunk by default.")
                else:
                    logger.debug("No active or original chunk, skipping.")
                    continue
            ...
            if restore_entities:
                ...
                try:
                    ...
                    if active_entities_chunk:
                        ...
                        logger.debug("Keeping active entities by default.")
...
def process_chunk(
    restored_region: EmptyRegion, active_chunk: Chunk, original_chunk: Chunk, args, claimed_chunks: set = set()
):
    if not args.exclude:
...
example:
    ...
    parser.add_argument(
        "active", help='path to active world directory. Must contain a "region" and an "entities" folder.'
    )
    parser.add_argument(
        "-e",
        "--exclude",
        ...
        choices=["claims"],
        default=[],
        help="indicate what features to exclude from restoration.",
    )
    parser.add_argument(
        "-b",
        "--boundary",
    ...
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="add some extra print messages to see active progress."
    )
