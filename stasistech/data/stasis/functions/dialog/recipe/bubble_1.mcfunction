...
tellraw @a[tag=sts_recipe_bubble_dialog] [{"text":"["},{"text":"ａｖＳＹＳ","color":"aqua"},{"text":"] "},{"text":"ＴＩＤＥＳ　ＩＮ　Ａ　ＢＵＢＢＬＥ","color":"dark_aqua","underlined":true,"hoverEvent":{"action":"show_text","value":[{"text":"Click for command"}]},"clickEvent":{"action":"suggest_command","value":"/trigger sts_tg_get_recipe_bubble"}}]
