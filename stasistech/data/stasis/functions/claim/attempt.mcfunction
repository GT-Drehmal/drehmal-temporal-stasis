...
# FX
# Success case is handled by stasis:claim/success
# Do not need to check sts_claims because openpac-claims claim would have failed regardless
execute if score #success sts_temp matches 0 at @s run function stasis:claim/playsound/fail
execute if score #success sts_temp matches 0 at @s run function stasis:claim/particles/fail