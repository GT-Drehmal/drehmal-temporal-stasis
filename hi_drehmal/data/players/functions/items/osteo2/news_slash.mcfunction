...
execute if score #mythic_pvp? const matches 1 run tag @a[tag=!osteo_immune] add temp_immune
...
execute as @e[type=player,tag=temp_immune,distance=..6] positioned ~-6 ~-2 ~-6 if entity @s[dx=11,dy=3,dz=11] positioned ~6 ~ ~6 run damage @s[tag=!osteo_immune] 35 minecraft:bonesdissolved
