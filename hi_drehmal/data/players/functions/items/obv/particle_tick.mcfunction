...
execute as @a if score @s uuid0 = #uuid0 temp if score @s uuid1 = #uuid1 temp if score @s uuid2 = #uuid2 temp if score @s uuid3 = #uuid3 temp run tag @s add temp.obv

function players:items/obv/particle_loop

tag @a remove temp.obv
