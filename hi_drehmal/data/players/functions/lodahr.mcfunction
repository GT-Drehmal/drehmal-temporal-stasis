...
## Forced progression: Only allows numencore access if all three tutorial facilities have been completed
execute unless score @s khive_pcool matches 1.. unless score #story_disabled? bool matches 1 if score #ring_facility bool matches 1 if score #inscriptionadvancement bool matches 1 if score #supplydepotadvancement bool matches 1 if entity @s[x=27391.0,y=125.0,z=53.0,dx=3,dy=5,dz=5,scores={rx=4500..13500,x2=..-25}] run function players:misc/core_portal2
execute unless score @s khive_pcool matches 1.. unless score #story_disabled? bool matches 1 if score #ring_facility bool matches 1 if score #inscriptionadvancement bool matches 1 if score #supplydepotadvancement bool matches 1 if score #worldtime x matches 70..190 if entity @s[x=425,y=4,z=-834,dx=2,dy=5,dz=5,scores={rx=-13500..-4500,x2=25..}] if block 440 3 -825 diamond_block run function players:misc/core_portal1
